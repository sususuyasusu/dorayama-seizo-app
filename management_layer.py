...
def get_dorayama_management():
    ...
    return {
        ...
        "expenseAudit": {
            ...
            #   件数が一致し全件仕訳済み。当初「口座の自動同期が2026-05-09で停止」と記載したが
            #   誤りだった（walletables APIのupdate_dateを同期実行日と誤読しただけで、実際は
            #   直近8/5まで明細が届き全件消込済みと確認・訂正済み）。帳簿残高との差
            #   23,963,739円はfreee API(walletables/wallet_txns)からは同期残高情報が
            #   取得できず特定できていない。freee画面での確認が必要。
            "unresolved": [
                ...
                {"label": "PayPay銀行（ジャパンネット口座）", "reason": "6月の入出金90件は件数一致・全件仕訳済みを確認。直近8月5日分まで明細も届いており同期自体は正常。ただし帳簿残高との差23,963,739円の内訳はAPIからは特定できず、freee画面での口座同期残高の確認が必要"},
            ],
        },
    }
